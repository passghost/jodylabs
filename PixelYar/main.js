...
function drawOcean() {
  ...
  // Layered blue noise texture
  for (let i = 0; i < 12000; i++) {
    ctx.fillStyle = `rgba(30,63,102,${Math.random() * 0.15 + 0.05})`;
    ctx.fillRect(Math.random() * OCEAN_WIDTH, Math.random() * OCEAN_HEIGHT, 2, 2);
  }
  for (let i = 0; i < 4000; i++) {
    ctx.fillStyle = `rgba(60,120,180,${Math.random() * 0.08 + 0.04})`;
    ctx.fillRect(Math.random() * OCEAN_WIDTH, Math.random() * OCEAN_HEIGHT, 1, 3);
  }
  // Draw islands
  for (const isl of ISLANDS) {
    ctx.save();
