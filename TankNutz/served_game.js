...
function drawAllies() {
  for (const ally of allies) {
    ...
    const effectiveScale = 0.6 * ally.scale;
...
function drawPowerups(){ const nowt = performance.now(); for (const p of powerups){ const ss = worldToScreen(p.x, p.y); const sx = ss.x, sy = ss.y; // draw animated pixel pill into cached canvas
    ...
    try{
      if ((p.type === 'heal' || p.type === 'journal') && p.journalKey && (p.journalKey.startsWith('collect-'))){
        // draw collect icon
        try{ drawCollectLowResIntoPowerup(_pctx, 32, 32, nowt + (p.phase*60), p.journalKey); }catch(_){ _p_drawTechCard(32, 32, p.hue, nowt + (p.phase * 60)); }
      } else if (p.type === 'heal' || p.type === 'journal'){
        try{ drawHeartLowResIntoPowerup(_pctx, 32, 32, nowt + (p.phase*60), p.heartVariant || 'classic'); }catch(_){ _p_drawTechCard(32, 32, p.hue, nowt + (p.phase * 60)); }
      } else if (p.type === 'mini'){
        // Draw mini tank icon
        _pctx.clearRect(0, 0, POWERUP_TILE_PX, POWERUP_TILE_PX);
        const cx = 32, cy = 32;
    ...
    }catch(_){ _p_drawTechCard(32, 32, p.hue, nowt + (p.phase * 60)); }
    // draw with bobbing
...
const dx = mouseWorldX - heli.x, dy = mouseWorldY - heli.y; heli.turretAngle = Math.atan2(dy, dx);
  ...
  if (dist < (e.r + 2)){
      ...
      bullets.splice(i,1);
      removed = true;
  if (e.harmless){ enemies.splice(ei,1); recordDeath(e.kind || '<harmless>', e); try{ recordKill(e.kind || (typeof e.spriteIdx === 'number' ? ('animal-' + e.spriteIdx) : '<harmless>'), e); }catch(_){ } score += 1; updateHud(); callSpawnGibs(e.x, e.y, '#ff8b6b', 8, 'enemy-hit-harmless'); const r = Math.random(); if (r < 0.15) spawnPowerup(e.x, e.y, 'double'); else if (r < 0.3) spawnMiniPowerup(e.x, e.y); }
      else if (e.type === 'jungle' || e.type === 'animal'){
        e.hp = (e.hp||1) - 1;
  if (e.hp <= 0){ enemies.splice(ei,1); score += 1; updateHud(); callSpawnGibs(e.x, e.y, '#ff8b6b', 12, 'enemy-killed-by-bullet'); const r = Math.random(); if (r < 0.15) spawnPowerup(e.x, e.y, 'double'); else if (r < 0.3) spawnMiniPowerup(e.x, e.y); }
  ...
  } else { enemies.splice(ei,1); try{ recordDeath(e.kind || '<default-killed>', e); }catch(_){ } try{ recordKill(e.kind || (typeof e.spriteIdx === 'number' ? ('animal-' + e.spriteIdx) : '<default-killed>'), e); }catch(_){ } score += 1; updateHud(); callSpawnGibs(e.x, e.y, '#ff8b6b', 10, 'enemy-default-killed'); const r = Math.random(); if (r < 0.15) spawnPowerup(e.x, e.y, 'double'); else if (r < 0.3) spawnMiniPowerup(e.x, e.y); }
      break;
    }
  }
  if (removed) continue;
