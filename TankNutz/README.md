...
#### Guardian Helicopter (Heli-Guard Powerup)
- **Rare Drop (1%)**: Spawns a non-journaled guardian helicopter ally for 5 real-time minutes
- **Orbiting Support AI**: Naturally orbits the player in a smooth elliptical path while rotating to face current target
- **Target Acquisition**: Continuously seeks nearest hostile within engagement radius
- **MIRV Missile Salvo**: Fires stylized MIRV-inspired homing micro-missiles on a timed cadence (≈650ms) using custom trail + color cycling effect
- **Missile Behavior**: Initial straight travel, then mild terminal homing + fading exhaust trail; rendered separately from standard bullets for future tuning
- **Pixel Swarm Replication**: In-game MIRV logic mirrors `MIrv.html` boid spread → converge phases with per-missile separation, convergence steering, speed limiting, 10-pixel trail, 2x4 body + black outline
- **Debug Key**: Press `B` to instantly spawn a guardian (Shift+B spawns a plain helicopter)
- **Safe Expiry**: Auto-despawns after duration without affecting other allies
- **Debug Spawn**: `spawnHeliGuardPowerup()` (attach in console) for quick testing
...
4. **Multi-shot**: Fire multiple bullets (upgrades to Triple/Quad/Pet)
5. **Mini Tanks**: Spawn AI allies (weapon level 5)
6. **Heli-Guard (New)**: 1% rare drop that summons a guardian helicopter ally for 5 minutes. Non-journaled collectible; use `spawnHeliGuardPowerup()` in console to test.
