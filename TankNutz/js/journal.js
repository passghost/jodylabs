...
function loadJournalState(){
  try{
    ...
    if (obj && obj.killCounts){
      // Only load powerup-like keys (we no longer persist generic kill entries)
      for (const k of Object.keys(obj.killCounts)){
        if (!k) continue;
        if (!(k.startsWith('collect-') || k.startsWith('powerup-') || k.startsWith('journal') || k.startsWith('set '))) continue;
...
function saveJournalState(){
  try{
    // Persist only powerup-like entries; do not save generic kill entries
    const payload = { killCounts: {} };
    for (const k of Object.keys(killCounts)){
      try{
        if (!k) continue;
        if (!(k.startsWith('collect-') || k.startsWith('powerup-') || k.startsWith('journal') || k.startsWith('set '))) continue;
        payload.killCounts[k] = { count: killCounts[k].count || 0, imgSrc: killCounts[k].imgSrc || null };
      }catch(_){ }
    }
    try{ localStorage.setItem(JOURNAL_KEY, JSON.stringify(payload)); }catch(_){ }
  }catch(_){ }
}
...
function ensureJournal(){
  ...
  try{
    resetBtn.addEventListener('click', ()=>{
      ...
      // clear all journal entries (we only store powerup-like keys now, but ensure a full clear)
        try{
          for (const k of Object.keys(killCounts)){
            try{ delete killCounts[k]; }catch(_){ }
          }
        }catch(_){ }
        try{ updateJournal(); }catch(_){ }
        // persist updated journal state
...
export function updateJournal(){
  ...
  // render powerup entries: show collected powerups (journalKey / powerup-related keys)
  // Keep killCounts as the backing store but only present powerup-like keys here
  const allKeys = Object.keys(killCounts || {});
  const powerupKeys = allKeys.filter(k => !!k && (k.startsWith('collect-') || k.startsWith('powerup-') || k.startsWith('journal') || k.startsWith('set ')));
  // Sort by count desc
  powerupKeys.sort((a,b)=>{
  ...
  je.innerHTML = '';
  for (const k of powerupKeys){
    const info = killCounts[k] || { count: 0, imgSrc: null };
    const el = document.createElement('div'); el.className = 'journal-entry';
    // prefer recorded image; if missing, draw a small powerup canvas using game helper
    if (info.imgSrc){ const im = document.createElement('img'); im.src = info.imgSrc; im.className = 'journal-entry-icon'; el.appendChild(im); }
    else {
      const c = document.createElement('canvas'); c.width = 32; c.height = 32; c.className = 'journal-entry-icon';
      try{
        ...
        if (window && typeof window.drawCollectLowResIntoPowerup === 'function'){
          ...
          cc.fillStyle = '#666'; cc.fillRect(0,0,32,32);
          cc.fillStyle = '#fff'; cc.font = '10px sans-serif'; cc.textAlign='center'; cc.fillText(k.replace(/^collect-/, '').slice(0,2).toUpperCase(), 16, 20);
        }
...
export function recordKill(kind, e){
  try{
    // Only accept powerup/collection/journal/set keys. Ignore generic kill entries.
    try{
      if (!kind || typeof kind !== 'string') return;
      if (!(kind.startsWith('collect-') || kind.startsWith('powerup-') || kind.startsWith('journal') || kind.startsWith('set '))) return;
    }catch(_){ return; }
    // Debug: log incoming journal records for troubleshooting
    try{
      if (typeof console !== 'undefined' && typeof console.debug === 'function'){
        try{ console.debug('journal.recordKill called:', kind, 'tileC?', !!(e && e.tileC), 'spriteIdx?', (e && e.spriteIdx)); }catch(_){ }
      }
    }catch(_){ }
    ...
    try{
      if (typeof console !== 'undefined' && typeof console.debug === 'function'){
        try{ console.debug('journal.recordKill updated:', k, killCounts[k] ? { count: killCounts[k].count, hasImg: !!killCounts[k].imgSrc } : null); }catch(_){ }
      }
    }catch(_){ }
    try{
      ...
      if (!killCounts[k].imgSrc){
        ...
        // If we still don't have an image, try to generate a small, specific thumbnail
        // using the shared game helper `drawCollectLowResIntoPowerup` where possible.
        try{
          if (!killCounts[k].imgSrc){
            // Known collect/powerup keys that have dedicated draw handlers
            const knownKeys = [
              'collect-saw', 'collect-peanut', 'collect-crescent',
              'powerup-double', 'powerup-mini', 'powerup-shield'
            ];
            const nk = (k || '').toLowerCase();
            if (knownKeys.includes(nk)){
              try{
                if (window && typeof window.drawCollectLowResIntoPowerup === 'function'){
                  ...
                  // call with small center coords (16,16)
                  window.drawCollectLowResIntoPowerup(tctx, 16, 16, performance.now(), nk);
                  killCounts[k].imgSrc = tc.toDataURL();
                } else {
                  // fallback to packaged asset if helper missing
                  const diskPath = 'Adimages/Jodyplaysfirstbetter22.png';
                  killCounts[k].imgSrc = diskPath;
                }
              }catch(_){
                // if thumbnail creation fails, fallback to asset
                try{ killCounts[k].imgSrc = 'Adimages/Jodyplaysfirstbetter22.png'; }catch(__){}
              }
            }
          }
        }catch(_){ }
