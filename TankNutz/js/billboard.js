...
function _extractDriveFileId(u){
  try{
...
function _normalizeDriveUrl(u){
  try{
    const id = _extractDriveFileId(u);
    if (id) return `https://drive.google.com/thumbnail?id=${id}&sz=w1000`;
  }catch(_){ }
  return u;
}

function _driveUrlVariants(u){
  const id = _extractDriveFileId(u);
  if (!id) return [_normalizeDriveUrl(u)];
  return [
...
async function _loadExternalAds(urls){
  if (!Array.isArray(urls) || !urls.length) return false;
  let loaded = 0;
  for (const raw of urls){
    const candidates = _driveUrlVariants(raw);
    let ok = false;
    for (const url of candidates){
      try{ const c = await _loadAdImage(url); if (c) { billboardAdCanvases.push(c); loaded++; ok = true; break; } }catch(_){ }
    }
    if (!ok){ /* skip if none worked */ }
  }
  return loaded > 0;
}

async function _loadDriveFolderViaApi(){
  try{
    const folderVal = (typeof window !== 'undefined') ? (window.BILLBOARD_DRIVE_FOLDER || window.BILLBOARD_DRIVE_FOLDER_ID) : null;
    const apiKey = (typeof window !== 'undefined') ? window.GDRIVE_API_KEY : null;
    if (!folderVal || !apiKey) return false;
    const folderId = (/^https?:/.test(folderVal)) ? (folderVal.match(/folders\/([^?]+)/) || [null,null])[1] : folderVal;
    ...
    const urls = files.map(f => `https://drive.google.com/thumbnail?id=${f.id}&sz=w1000`);
    return _loadExternalAds(urls);
...
async function _loadAdImage(url){ return new Promise((resolve)=>{ try{ const img = new Image(); try{ img.referrerPolicy = 'no-referrer'; }catch(_){ } img.onload = ()=>{ try{ const c = document.createElement('canvas'); c.width = img.width; c.height = img.height; const g = c.getContext('2d'); g.imageSmoothingEnabled = false; g.drawImage(img,0,0); resolve(c); }catch(_){ resolve(null); } }; img.onerror = ()=>resolve(null); img.src = url; }catch(_){ resolve(null); } }); }
...
export async function loadAdImages(){
  ...
  // Precedence override: 0) global external list  1) Drive API (optional key)  2) drive_manifest.json
  try{
    if (typeof window !== 'undefined' && Array.isArray(window.__BILLBOARD_EXTERNAL_ADS) && window.__BILLBOARD_EXTERNAL_ADS.length){
      const ok = await _loadExternalAds(window.__BILLBOARD_EXTERNAL_ADS);
      if (ok) return billboardAdCanvases;
    }
  }catch(_){ }
  try{ const apiOk = await _loadDriveFolderViaApi(); if (apiOk) return billboardAdCanvases; }catch(_){ }
  try{
    // optional local manifest specifically for external/drive urls
    const resp = await fetch('Adimages/drive_manifest.json');
    if (resp && resp.ok){ const list = await resp.json(); if (Array.isArray(list) && list.length){ const ok = await _loadExternalAds(list); if (ok) return billboardAdCanvases; } }
  }catch(_){ }
