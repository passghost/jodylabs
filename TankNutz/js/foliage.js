...
export function initFoliage(worldW, worldH){ WORLD_W = worldW; WORLD_H = worldH; grassCanvas = document.createElement('canvas'); grassCanvas.width = WORLD_W; grassCanvas.height = WORLD_H; gctx = grassCanvas.getContext('2d'); buildGrass(); buildFoliageTiles(); scatterFoliage(WORLD_W, WORLD_H); foliageReady = true; }


// debugging probes removed

export function updateFoliage(now, windA, windMag){ // update animated tile frames
  // Throttle foliage tile updates to avoid expensive per-frame redraws.
  // Animated tiles (like jungle trees) don't need 60fps; 20fps is sufficient.
  if (typeof updateFoliage._last === 'undefined') updateFoliage._last = 0;
  const THROTTLE_MS = 50; // ~20 FPS
  if (now - updateFoliage._last < THROTTLE_MS) return;
  updateFoliage._last = now;
...
export function drawFoliage(ctx, worldToScreen, camera){ // draw grass backdrop
  // Avoid noisy per-frame logging which can bloat the console and slow down
  // the game when DevTools is open.
