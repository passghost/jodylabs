...
export function makeSquidRobotIdle(g){
  // Canonical squidrobot builder (pixel-spec from Squidrobot.html)
  const SRC = 64, CX = 32, CY = 32, FRAMES = 12;
  const bodyFrames = [];
  const bodyBob = [];
  function makeLayer(w=SRC,h=SRC){ const c = document.createElement('canvas'); c.width = w; c.height = h; return c; }
  function pxLocal(ctx,x,y,col,t=1){ ctx.fillStyle = col; ctx.fillRect(Math.round(x), Math.round(y), t, t); }
  function lineLocal(ctx,x0,y0,x1,y1,col,t=1){ x0=Math.round(x0); y0=Math.round(y0); x1=Math.round(x1); y1=Math.round(y1); const dx=Math.abs(x1-x0), sx=x0<x1?1:-1; const dy=-Math.abs(y1-y0), sy=y0<y1?1:-1; let err=dx+dy; while(true){ pxLocal(ctx,x0,y0,col,t); if(x0===x1 && y0===y1) break; const e2=2*err; if(e2>=dy){ err+=dy; x0+=sx; } if(e2<=dx){ err+=dx; y0+=sy; } } }
  function circleFillLocal(ctx,cx,cy,r,col){ ctx.fillStyle = col; for(let y=-r;y<=r;y++){ const w = Math.floor(Math.sqrt(r*r - y*y)); ctx.fillRect(Math.round(cx - w), Math.round(cy + y), 2*w+1, 1); } }
  function rimLightArcLocal(ctx,cx,cy,r,col){ ctx.fillStyle = col; for(let a=-0.2;a<=0.9;a+=0.05){ const x = Math.round(cx + Math.cos(a)*r); const y = Math.round(cy + Math.sin(a)*r); ctx.fillRect(x, y, 1, 1); if (Math.random()<0.3) ctx.fillRect(x, y-1, 1, 1); } }
  function dottedBarrelLocal(ctx, x0, y0, angle, length, color, t=2){ const dx = Math.cos(angle), dy = Math.sin(angle); for (let s=2; s<=length; s++){ const x = Math.round(x0 + dx*s); const y = Math.round(y0 + dy*s); pxLocal(ctx, x, y, color, t); if (s%3===0) pxLocal(ctx, x, y, color, t); } const mx = Math.round(x0 + dx*(length+1)); const my = Math.round(y0 + dy*(length+1)); pxLocal(ctx, mx-1, my-1, '#c9d1d9', t); }
  function legLocal(ctx,cx,cy,baseAngle,phase){ const L1=7, L2=6; const swing = Math.sin(phase)*2.0; const lift = Math.cos(phase*0.5)*1.0; const bodyR = 12; const bx = cx + Math.cos(baseAngle)*(bodyR - 1); const by = cy + Math.sin(baseAngle)*(bodyR - 1); const kx = bx + Math.cos(baseAngle)*(L1 + swing*0.4); const ky = by + Math.sin(baseAngle)*(L1 + swing*0.4) - lift; const fx = kx + Math.cos(baseAngle)*(L2 + swing); const fy = ky + Math.sin(baseAngle)*(L2 + swing) + lift*0.25; lineLocal(ctx, bx, by, kx, ky, '#1e2430', 2); lineLocal(ctx, kx, ky, fx, fy, '#5e7389', 1); pxLocal(ctx, fx, fy, '#5e7389', 2); }
  function shadowLocal(ctx){ ctx.save(); ctx.fillStyle = 'rgba(0,0,0,0.18)'; const rx = 16, ry = 6, cy = CY + 10; for (let y=-ry; y<=ry; y++){ const w = Math.floor(Math.sqrt(1 - (y*y)/(ry*ry)) * rx); ctx.fillRect(CX - w, cy + y, 2*w+1, 1); } ctx.restore(); }
  for (let fi=0; fi<FRAMES; fi++){
    ...
    shadowLocal(ctx);
    for (let i=0; i<8; i++){ const a = (i/8) * Math.PI*2; const phase = (t * Math.PI*2) + (i%2 ? Math.PI : 0); legLocal(ctx, CX, CY + bob, a, phase); }
    // legacy three-disc body
    circleFillLocal(ctx, CX, CY + bob, 12, '#11141a');
    circleFillLocal(ctx, CX, CY + bob, 11, '#1e2430');
    circleFillLocal(ctx, CX, CY + bob, 9,  '#2b3445');
    rimLightArcLocal(ctx, CX-2, CY-2 + bob, 10, '#5e7389');
    bodyFrames.push(c); bodyBob.push(bob);
  }
  // Draw turret into the target ctx, but account for scale+offset so it centers when the body
  // frame was scaled into CHAR_TILE_PX. We accept scale and dst offsets (dx,dy) used when
  // drawing the scaled body frame so turret pixels align exactly.
  function drawTurretTo(ctx, angle, bob, scale = 1, dx = 0, dy = 0){
    // transform to scaled body coordinate space, draw using original SRC coordinates
    ctx.save();
    ctx.translate(dx, dy);
    ctx.scale(scale, scale);
    const cx = CX, cy = CY - 1 + bob;
    circleFillLocal(ctx, cx, cy, 6, '#1e2430');
    circleFillLocal(ctx, cx, cy, 5, '#2b3445');
    pxLocal(ctx, cx-1, cy-3, '#5e7389', 2);
    for (let k=0; k<6; k++){ const aa = angle + (k/6)*Math.PI*2; const bx = Math.round(cx + Math.cos(aa)*4); const by = Math.round(cy + Math.sin(aa)*4); pxLocal(ctx, bx, by, '#8fb4cf', 1); }
    dottedBarrelLocal(ctx, cx, cy, angle, 14, '#5e7389', 2);
    const sx = Math.round(cx + Math.cos(angle)*8 - Math.sin(angle)*2);
    const sy = Math.round(cy + Math.sin(angle)*8 + Math.cos(angle)*2);
    pxLocal(ctx, sx, sy, '#c9d1d9', 1);
    ctx.restore();
  }
  return (t, lookA) => {
    // external override
    if (ASSETS && ASSETS.squid){ const img = ASSETS.squid; const scale = Math.floor((CHAR_TILE_PX * 0.92) / Math.max(1, img.width)); const dw = img.width * scale, dh = img.height * scale; const dx = Math.round((CHAR_TILE_PX - dw)/2); const dy = Math.round((CHAR_TILE_PX - dh)/2 + 2); g.clearRect(0,0,CHAR_TILE_PX,CHAR_TILE_PX); g.imageSmoothingEnabled = false; g.drawImage(img, 0,0, img.width, img.height, dx, dy, dw, dh); return; }
    const idx = Math.floor((t/100) % bodyFrames.length); const frame = bodyFrames[idx]; const bob = bodyBob[idx] || 0;
    // draw scaled
    const scale = Math.floor((CHAR_TILE_PX * 0.92) / Math.max(1, SRC)); const dw = SRC * scale, dh = SRC * scale; const dx = Math.round((CHAR_TILE_PX - dw)/2); const dy = Math.round((CHAR_TILE_PX - dh)/2 + 2);
  g.clearRect(0,0,CHAR_TILE_PX,CHAR_TILE_PX); g.imageSmoothingEnabled = false; g.drawImage(frame, 0,0, SRC, SRC, dx, dy, dw, dh);
  drawTurretTo(g, lookA, bob, scale, dx, dy);
  };
}
export function makeSquidRobotRun(g){ return makeSquidRobotIdle(g); }

// Kraken maker
export function makeKrakenIdle(g){
  // Full animated kraken (6 frames) with tentacles, barrel, eyes, glacis wedge – adapted from legacy gameold.js.
  ...
  function tentacle(ctx,cx,cy,baseAngle,tPhase,bodyRadius,frame){
    const segs = 9;
    for(let s=0;s<segs;s++){
      const progress = s/(segs-1);
      const throb = Math.sin((frame + s*0.6 + tPhase) * 0.9);
      const bend = 0.35 * throb * (0.3 + 0.7*progress);
      ...
      const col = (s < 2) ? '#4f6a6d' : (s < 5 ? '#2f4f4f' : '#0a2a2f');
      for(let dy=0;dy<size;dy++) for(let dx=0;dx<size;dx++) px(ctx, x + dx - (size>>1), y + dy - (size>>1), col);
      if(size >= 2 && (s % 2 === 0)){
  ...
  for (let fi=0; fi<FRAMES; fi++){
    ...
    for (let y=-14; y<=14; y++){
      ...
      for (let x=-span; x<=span; x++){
        const rr = x*x + y*y;
        if (rr <= 14*14 && rr >= (14*14 - 18) && (x + y) < -4) px(ctx, cx + x, cy + y, '#8fe3f7');
      }
    }
    // Central dome + panel
    ...
    // Barrel integrated (kept static forward)
    ...
    for (let k=0;k<8;k++) tentacle(ctx, cx, cy, k*(Math.PI/4), k*0.7, baseR, fi);
    // Glacis wedge accent
    for (let i=0;i<6;i++){
      const a = angle; const r = baseR - 2 + i; const x = Math.round(cx + Math.cos(a)*r); const y = Math.round(cy + Math.sin(a)*r);
      px(ctx,x,y,'#4f6a6d'); if(i<4) px(ctx, x + Math.round(Math.cos(a+Math.PI/2)), y + Math.round(Math.sin(a+Math.PI/2)), '#4f6a6d');
    }
  ...
  return (t, lookA) => {
    if (ASSETS.kraken){
      ...
      const dw = srcW * scale, dh = srcH * scale; const dx = Math.round((CHAR_TILE_PX - dw)/2); const dy = Math.round((CHAR_TILE_PX - dh)/2 + 2);
      g.clearRect(0,0,CHAR_TILE_PX,CHAR_TILE_PX); g.imageSmoothingEnabled = false; g.drawImage(img,0,0,srcW,srcH,dx,dy,dw,dh); return;
    }
    const idx = Math.floor((t/120) % frames.length); const frame = frames[idx];
    g.clearRect(0,0,CHAR_TILE_PX,CHAR_TILE_PX); g.drawImage(frame,0,0);
  };
}
...
// register maps
export const idleMakers = { squidrobot: makeSquidRobotIdle, kraken: makeKrakenIdle, osprey: makeOspreyIdle };
export const runMakers = { squidrobot: makeSquidRobotRun, kraken: makeKrakenRun, osprey: makeOspreyRun };
