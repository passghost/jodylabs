...
export const JUNGLE_KINDS = ['commando','scout','heavy','medic','villager1','villager2','guide','radioop','fatsammich','squidrobot','kraken','kittytank'];
...
// simple placeholder "kitty" maker: pulsing/popping circle used as a temporary enemy tile (legacy palette removed)
export function makeKittyIdle(g, ASSETS){
  // Synthesize composite helper early if only body/turret helpers are present.
  try{
    if (typeof globalThis !== 'undefined' && typeof globalThis.drawKittenTile !== 'function' && typeof globalThis.drawKittenBody === 'function'){
      globalThis.drawKittenTile = function(ctx, t){
        try{ globalThis.drawKittenBody(ctx, t); }catch(_){ }
        try{ if (typeof globalThis.drawKittenTurret === 'function') globalThis.drawKittenTurret(ctx, t); }catch(_){ }
      };
    }
  }catch(_){ }
  return (t, lookA) => {
    ...
    try{
      // Proactively attempt dynamic import of alternativetanks if the helper isn't loaded yet
      if (!makeKittyIdle._attemptedAltImport && (typeof globalThis !== 'undefined') && !globalThis.drawKittenTile){
        makeKittyIdle._attemptedAltImport = true;
        try{ import('./alternativetanks.js').catch(()=>{}); }catch(_){ }
      }
        // Diagnostic: record availability of draw helpers
        try{
          if (typeof window !== 'undefined') window.__kitty_debug = window.__kitty_debug || {};
          if (typeof globalThis !== 'undefined'){
            try{ window.__kitty_debug.drawKittenTile = typeof globalThis.drawKittenTile === 'function'; }catch(_){ }
            try{ window.__kitty_debug.drawKittenBody = typeof globalThis.drawKittenBody === 'function'; }catch(_){ }
            try{ window.__kitty_debug.drawKittenTurret = typeof globalThis.drawKittenTurret === 'function'; }catch(_){ }
          }
        }catch(_){ }
        if (typeof globalThis !== 'undefined' && typeof globalThis.drawKittenTile === 'function'){
          ...
          try{
            // call and then verify pixels were drawn; store result for diagnostics
            try{ globalThis.drawKittenTile(g, t || performance.now()); }catch(err){ throw err; }
            try{
              if (typeof window !== 'undefined') window.__kitty_last_draw_info = window.__kitty_last_draw_info || {};
              try{
                const img = g.getImageData(0,0,g.canvas.width,g.canvas.height).data; let any=false; for (let i=3;i<img.length;i+=4){ if (img[i]!==0){ any=true; break; } }
                if (typeof window !== 'undefined') window.__kitty_last_draw_info.hasPixels = any;
              }catch(err2){ if (typeof window !== 'undefined') window.__kitty_last_draw_info.getImageDataError = String(err2); }
              if (typeof window !== 'undefined') window.__kitty_last_draw_info.drawCalled = true;
            }catch(_){ }
            return;
          }catch(_){ /* fallback placeholder removed */ }
        }
    }catch(_){ }
    // defensive check: if an external draw function painted nothing (silent failure), ensure we draw a fallback
    try{
      try{
  if (g && g.canvas){ const img = g.getImageData(0,0,g.canvas.width,g.canvas.height).data; let any=false; for (let i=3;i<img.length;i+=4){ if (img[i]!==0){ any=true; break; } } if (!any){ /* fallthrough to placeholder below */ } else { return; } }
      }catch(_){ /* ignore getImageData limitations, continue to fallback */ }
    }catch(_){ }
  // Last resort (should rarely execute now): neutral gray square instead of prior placeholder
  try{ g.fillStyle = '#555'; g.fillRect(W*0.3, H*0.3, W*0.4, H*0.4); }catch(_){ }
  };
}
export function makeKittyRun(g, ASSETS){
  const idle = makeKittyIdle(g, ASSETS);
  return (t, lookA) => {
...
// register kittytank makers so game.js can spawn this kind and it will behave like other jungle NPCs
idleMakers.kittytank = makeKittyIdle;
runMakers.kittytank = makeKittyRun;
