...
function handleFoliageHit(fiIndex, projX, projY, color = '#8adf76'){
  try{
    ...
    if (f.hp <= 0){
      ...
      // Drop powerups from destroyed foliage using staged system
      tryDropEnvironmentalPowerup(f.x, f.y, 'foliage');
...
// pet tanks (level 5 weapon upgrade)
const petTanks = []; // {x,y,vx,vy,angle,turretAngle,color,lastShot,scale}
...
function spawnMiniTankAlly(x, y) {
  ...
  const miniScale = 0.25; // Tiny pet size
  const offsetX = 20; // Small offset for visibility
  const offsetY = 20;

  // List of all available tank variants
  const availableVariants = ['default', 'fordfiesta', 'german', 'mexico', 'china', 'mcds', 'murkia', 'dozer', 'bondtank', 'french', 'facebook', 'waffle', 'blackstar'];

  // Get player's current variant
  const playerVariant = selectedVehicleVariant || 'default';

  // Filter out the player's current variant to ensure different color
  const differentVariants = availableVariants.filter(variant => variant !== playerVariant);

  // Randomly select a different variant
  const randomVariant = differentVariants[Math.floor(Math.random() * differentVariants.length)] || 'default';

  const ally = {
    x: mod(x + offsetX, WORLD_W),
    y: mod(y + offsetY, WORLD_H),
    vx: (Math.random() - 0.5) * 10, // Small random initial velocity
    vy: (Math.random() - 0.5) * 10,
    spd: tank.speed, // Match player's base speed
    r: 6, // Smaller hit radius for tiny size
    type: 'miniTank',
    hp: 5, // Reduced for tiny size
    lastAttack: 0,
    attackCD: 800 + Math.random() * 400, // Similar to jungle enemies
    turretAngle: 0,
    angle: 0,
    scale: miniScale,
    tankVariant: randomVariant, // Always use a different variant than player
    spawnTime: performance.now(), // For 60 second duration
    // Visual properties
    treadPhase: 0,
    dust: 0
  };

  // Limit to 3 tiny allies max
  if (allies.length < 3) {
    allies.push(ally);
  }
}
...
function updateAllies(dt) {
  ...
  for (let i = allies.length - 1; i >= 0; i--) {
    ...
    for (const enemy of enemies) {
      ...
      const dx = wrapDeltaX(enemy.x, ally.x);
      const dy = wrapDeltaY(enemy.y, ally.y);
    ...
    let speedBoost = shiftHeld ? 1.3 : 1.0;
    if (shiftHeld && selectedVehicleVariant === 'fordfiesta') {
      speedBoost = 2.8;
    }
    ally.spd = tank.speed * speedBoost;
    ...
    if (shiftHeld) {
      // Seek player
      const dx = wrapDeltaX(playerX, ally.x);
      const dy = wrapDeltaY(playerY, ally.y);
      const dist = Math.hypot(dx, dy);
      if (dist > 5) {
      ...
      // Fight: move towards nearest enemy
      if (nearestEnemy && nearestDist < 400) {
        const dx = wrapDeltaX(nearestEnemy.x, ally.x);
        const dy = wrapDeltaY(nearestEnemy.y, ally.y);
        const dist = Math.hypot(dx, dy);
        if (dist > 0) {
          ally.vx = (dx / dist) * ally.spd;
          ally.vy = (dy / dist) * ally.spd;
    ...
    // Point turret at nearest enemy if exists
    ...
    ally.x = mod(ally.x, WORLD_W);
    ally.y = mod(ally.y, WORLD_H);
    ...
    // Shooting logic
    if (nearestEnemy && nearestDist < 250 && now - ally.lastAttack > ally.attackCD) {
      // Shoot at enemy
...
// --- Pet Tank System (Level 5 Weapon Upgrade) ---
function spawnPetTank(x, y) {
  // Clear any existing pet tanks (only one pet tank at a time)
  petTanks.length = 0;
  ...
  const pet = {
    x: x + (Math.random() - 0.5) * 60, // Spawn near player
    y: y + (Math.random() - 0.5) * 60,
    vx: 0,
    vy: 0,
    angle: 0, // Tank body angle
    turretAngle: 0, // Turret rotation
    color: color,
    scale: 0.25, // Quarter size
    lastShot: 0,
    shotCooldown: 600, // Faster than player (600ms vs ~800ms)
    followDistance: 30 + Math.random() * 20, // Random follow distance
    followAngle: Math.random() * Math.PI * 2, // Random position around player
    treadPhase: 0,
    dust: 0
  };
...
function updatePetTanks(dt) {
  const now = performance.now();
  const playerX = selectedVehicle === 'heli' ? heli.x : tank.x;
  const playerY = selectedVehicle === 'heli' ? heli.y : tank.y;
  
  for (let i = petTanks.length - 1; i >= 0; i--) {
    const pet = petTanks[i];
    
    // Update follow position around player
    pet.followAngle += dt * 0.5; // Slowly orbit around player
    const targetX = playerX + Math.cos(pet.followAngle) * pet.followDistance;
    const targetY = playerY + Math.sin(pet.followAngle) * pet.followDistance;
    ...
    if (dist > 5) {
      const speed = 40; // Slightly slower than player
    ...
    pet.dust = Math.max(0, pet.dust - dt * 2);
    if (speed > 10) {
      pet.dust = Math.min(1, pet.dust + dt * 0.5);
    }
    
    // Find nearest enemy for turret aiming
    let nearestEnemy = null;
    let nearestDist = Infinity;
    for (const enemy of enemies) {
      if (enemy.harmless) continue;
      const enemyDx = enemy.x - pet.x;
      const enemyDy = enemy.y - pet.y;
      const enemyDist = Math.hypot(enemyDx, enemyDy);
      if (enemyDist < nearestDist && enemyDist < 200) { // Only target enemies within 200 units
        nearestDist = enemyDist;
        nearestEnemy = enemy;
      }
    }
    
    // Update turret angle to aim at nearest enemy
    if (nearestEnemy) {
      const enemyDx = nearestEnemy.x - pet.x;
      const enemyDy = nearestEnemy.y - pet.y;
      pet.turretAngle = Math.atan2(enemyDy, enemyDx);
      
      // Shoot at enemy
      if (now - pet.lastShot > pet.shotCooldown) {
        const bulletSpeed = 200;
        const bulletDx = Math.cos(pet.turretAngle) * bulletSpeed;
        const bulletDy = Math.sin(pet.turretAngle) * bulletSpeed;
        
        // Spawn bullet from turret position
        const turretOffset = 8 * pet.scale;
        const bulletX = pet.x + Math.cos(pet.turretAngle) * turretOffset;
        const bulletY = pet.y + Math.sin(pet.turretAngle) * turretOffset;
        ...
        bullets.push({
          ...
          dy: bulletDy,
          life: 2.0,
          color: pet.color // Use pet's color for bullets
        });
        
        pet.lastShot = now;
        try { spawnMuzzleFlash(bulletX, bulletY, pet.turretAngle); } catch(_) {}
      }
    } else {
      // No enemy nearby, aim turret in movement direction
      pet.turretAngle = pet.angle;
    }
    
    // Remove pet tank if too far from player (powerup expired)
    const distFromPlayer = Math.hypot(pet.x - playerX, pet.y - playerY);
...
function drawPetTanks(ctx, worldToScreen, camera) {
  for (const pet of petTanks) {
    try {
      // Convert world coordinates to screen coordinates
      const screenPos = worldToScreen(pet.x, pet.y);
      const sx = screenPos.x;
      const sy = screenPos.y;
      
      // Calculate effective scale for pet tank (quarter size)
      const effScale = pet.scale * camera.zoom;
      ...
      ctx.translate(-bodyCanvas.width/2, -bodyCanvas.height/2);
      ctx.imageSmoothingEnabled = false;
      
      // Apply pet's color tint
      ...
      ctx.save();
      ctx.translate(sx, sy);
      ctx.rotate(pet.turretAngle + Math.PI/2);
      ctx.scale(effScale, effScale);
      ctx.translate(-turretCanvas.width/2, -turretCanvas.height/2);
      ctx.imageSmoothingEnabled = false;
      
      // Apply pet's color tint to turret
      ctx.globalCompositeOperation = 'source-atop';
      ctx.fillStyle = pet.color;
      ctx.fillRect(0, 0, turretCanvas.width, turretCanvas.height);
      ctx.globalCompositeOperation = 'source-over';
      
      ctx.drawImage(turretCanvas, 0, 0);
      ctx.restore();
      
      // Draw dust effect
      if (pet.dust > 0) {
        ctx.save();
        ctx.globalAlpha = pet.dust * 0.6;
        ctx.fillStyle = '#8B7355';
        for (let i = 0; i < 3; i++) {
          const dustX = sx + (Math.random() - 0.5) * 20;
          const dustY = sy + (Math.random() - 0.5) * 20;
          ctx.beginPath();
      ...
      // Fallback: draw simple colored circle if canvas drawing fails
      const screenPos = worldToScreen(pet.x, pet.y);
      ctx.save();
      ctx.fillStyle = pet.color;
      ctx.beginPath();
...
function drawAllies() {
  for (const ally of allies) {
    // Handle world wrapping for drawing
    let drawX = ally.x - camera.x;
    let drawY = ally.y - camera.y;
    if (drawX > WORLD_W / 2) drawX -= WORLD_W;
    if (drawX < -WORLD_W / 2) drawX += WORLD_W;
    // For y, since it's wrapping, but camera.y is 0, perhaps not needed

    // Draw mini tank using simple shapes
    const effectiveScale = ally.scale * (camera.zoom || 1);
    ...
    ctx.fillStyle = '#f00'; // Red for testing
...
function tryDropEnvironmentalPowerup(x, y, sourceType = 'building') {
  ...
  if (rand < dropChance) {
    ...
    // Spawn the powerup
    if (powerupType === 'mini') {
      spawnMiniPowerup(x, y);
    } else if (powerupType === 'shield') {
      spawnShieldPowerup(x, y);
    } else if (powerupType === 'heal') {
      spawnHealPowerup(x, y);
    } else {
...
function spawnHealPowerup(x,y){ spawnPowerup(x,y,'heal'); try{ const p = powerups[powerups.length-1]; if (p){ p.hue = 140; p.heartVariant = 'classic'; } }catch(_){ } }
...
function spawnJournalPowerup(x,y, key){ spawnPowerup(x,y,'journal'); try{ const p = powerups[powerups.length-1]; if (p){ p.hue = 280; p.heartVariant = 'mend'; p.journalKey = key || 'collect-saw'; } }catch(_){ } }
...
function ensureHudInView(){
  try{
    const padding = 8;
    const winW = (window.innerWidth || document.documentElement.clientWidth);
    const winH = (window.innerHeight || document.documentElement.clientHeight);
    const clampTop = (r)=> Math.max(padding, Math.min(r, winH - padding));
    ...
    els.forEach(el => {
      if (!el) return;
      try{
        // Ensure element uses absolute pixel positioning for adjustment
        const rect = el.getBoundingClientRect();
        // compute top clamp
        let top = rect.top;
        // If element's height would overflow bottom, move it up
        if (rect.top + rect.height > winH - padding) top = Math.max(padding, winH - rect.height - padding);
        if (rect.top < padding) top = padding;
        el.style.top = Math.round(top) + 'px';

        // Ensure element is not off left/right edges. Prefer keeping 'right' if currently set.
        const computed = window.getComputedStyle(el);
        const hasRight = computed.right && computed.right !== 'auto';
        const hasLeft = computed.left && computed.left !== 'auto';
        // If element is off the right edge, set right to padding
        if (rect.right > winW - padding){ el.style.right = padding + 'px'; el.style.left = 'auto'; }
        // If element is off the left edge, set left to padding
        if (rect.left < padding){ el.style.left = padding + 'px'; el.style.right = 'auto'; }

        // If still overflowing horizontally, shrink via maxWidth
...
function recordDeath(kind, e) { 
  ...
  // Track wave progress
  if (waveInProgress && e && (e.type === 'jungle' || e.type === 'animal' || e.type === 'critter' || e.type === 'osprey')) {
    waveEnemiesDefeated++;
  }
}
...
const WAVE_CONFIGS = {
  1: { enemies: ['commando', 'scout'], count: 5, chainedChance: 0 },
  2: { enemies: ['commando', 'scout', 'medic'], count: 8, chainedChance: 0 },
  3: { enemies: ['commando', 'scout', 'medic', 'villager1', 'villager2'], count: 10, chainedChance: 0 },
  4: { enemies: ['commando', 'scout', 'heavy', 'medic', 'guide'], count: 12, chainedChance: 0.1 },
  5: { enemies: ['commando', 'scout', 'heavy', 'medic', 'guide', 'radioop'], count: 15, chainedChance: 0.15 },
  6: { enemies: ['commando', 'scout', 'heavy', 'medic', 'guide', 'radioop', 'fatsammich'], count: 18, chainedChance: 0.2 },
  7: { enemies: ['commando', 'scout', 'heavy', 'medic', 'guide', 'radioop', 'fatsammich', 'squidrobot'], count: 20, chainedChance: 0.25 },
  8: { enemies: ['commando', 'scout', 'heavy', 'medic', 'guide', 'radioop', 'fatsammich', 'squidrobot', 'kraken'], count: 22, chainedChance: 0.3 },
  9: { enemies: ['commando', 'scout', 'heavy', 'medic', 'guide', 'radioop', 'fatsammich', 'squidrobot', 'kraken', 'pinkdot'], count: 25, chainedChance: 0.4 },
  10: { enemies: ['heavy', 'guide', 'radioop', 'squidrobot', 'kraken', 'pinkdot'], count: 30, chainedChance: 0.6 }
};
...
addEventListener('keydown', e => { try{ if (e.key && e.key.toLowerCase() === 't'){ console.log('T pressed, spawning mini tank'); const playerX = (selectedVehicle === 'heli') ? heli.x : tank.x; const playerY = (selectedVehicle === 'heli') ? heli.y : tank.y; spawnMiniTankAlly(playerX + 60, playerY); console.log('Spawned mini tank ally for testing, allies count:', allies.length); } }catch(_){ console.log('Error spawning mini tank:', _); } });
...
function startNewWave() {
  ...
  console.log(`Starting Wave ${currentWave} with ${WAVE_CONFIGS[currentWave].count} enemies`);
...
function checkWaveProgress() {
  // Count defeated enemies (enemies that are no longer in the array)
  const currentEnemyCount = enemies.length;
  const expectedTotal = WAVE_CONFIGS[currentWave].count;
  ...
  if (waveEnemiesSpawned >= expectedTotal && (defeatRatio >= 0.7 || waveActiveTime >= 30)) {
    completeWave();
  }
}
...
function completeWave() {
  ...
  if (currentWave > MAX_WAVES) {
    ...
    // You could add victory logic here
  } else {
    // Reset for next wave
    waveEnemiesSpawned = 0;
    waveEnemiesDefeated = 0;
...
function spawnEnemy(){
  ...
  if (!waveInProgress && currentWave <= MAX_WAVES) {
    startNewWave();
    return;
  }
  
  // If wave is complete, don't spawn more enemies
  if (waveEnemiesSpawned >= WAVE_CONFIGS[currentWave].count) {
    return;
  }
  ...
  if (kind === 'heavy' && Math.random() < waveConfig.chainedChance) {
    try{ 
  ...
  const hpMap = { commando:3, scout:2, heavy:4, medic:2, villager1:1, villager2:1, guide:3, radioop:2, squidrobot:2, fatsammich:1, kraken:4, pinkdot:2 };
  const spdMap = { commando:45, scout:65, heavy:35, medic:50, villager1:40, villager2:40, guide:55, radioop:40, squidrobot:18, fatsammich:30, kraken:25, pinkdot:60 };
  ...
  if (kind === 'pinkdot'){
    ...
    try{ if (typeof globalThis !== 'undefined' && typeof globalThis.attachKittenRenderer === 'function') globalThis.attachKittenRenderer(ent); }catch(_){ }
  }
...
function restart(){ 
  ...
  waveEnemiesSpawned = 0;
  waveEnemiesDefeated = 0;
  waveInProgress = false; // Let startNewWave handle this
  waveStartTime = 0;
...
function updateHud(){
  ...
  // environmental powerup cooldown indicator
  let envCooldownInfo = '';
  ...
  // left side: organize all status elements vertically
  let leftContent = `<div class="tn-bolts" title="bolted"></div>`;

  // Add powerup indicator to left side if active
  if (pu) {
    leftContent += `<div class="tn-status-item">${pu}</div>`;
  }

  // Add shield indicator to left side if active
  if (shieldInfo) {
    leftContent += `<div class="tn-status-item">${shieldInfo}</div>`;
  }

  // Add environmental cooldown to left side if active
  if (envCooldownInfo) {
    leftContent += `<div class="tn-status-item">${envCooldownInfo}</div>`;
  }

  const leftHTML = leftContent;

  // Keep right panel compact: only health
  ...
  // Ensure HUD floating elements are inside viewport and stay visible
  ...
  try{
    ...
    `;
    // Append weapon phase and cooldown elements below the language button
    let phaseDiv = document.getElementById('tn-phase-div');
    if (!phaseDiv) {
      phaseDiv = document.createElement('div');
      phaseDiv.id = 'tn-phase-div';
      phaseDiv.className = 'tn-phase-div';
      ctrl.appendChild(phaseDiv);
    }
    let cooldownDiv = document.getElementById('tn-cooldown-div');
    if (!cooldownDiv) {
      cooldownDiv = document.createElement('div');
      cooldownDiv.id = 'tn-cooldown-div';
      cooldownDiv.className = 'tn-cooldown-div';
      ctrl.appendChild(cooldownDiv);
    }
    let powerupDiv = document.getElementById('tn-powerup-div');
    if (!powerupDiv) {
      powerupDiv = document.createElement('div');
      powerupDiv.id = 'tn-powerup-div';
      powerupDiv.className = 'tn-powerup-div';
      ctrl.appendChild(powerupDiv);
    }
    let waveDiv = document.getElementById('tn-wave-div');
    if (!waveDiv) {
      waveDiv = document.createElement('div');
      waveDiv.id = 'tn-wave-div';
      waveDiv.className = 'tn-wave-div';
      ctrl.appendChild(waveDiv);
    }
    let scoreDiv = document.getElementById('tn-score-div');
    if (!scoreDiv) {
      scoreDiv = document.createElement('div');
      scoreDiv.id = 'tn-score-div';
      scoreDiv.className = 'tn-score-div';
      ctrl.appendChild(scoreDiv);
    }
    // wire handlers
  ...
  function openLangMenu(){ try{ try{ console.log('DEBUG: local openLangMenu called'); }catch(_){ } closeLangMenu(); const menu = document.createElement('div'); menu.id = 'tn-lang-menu'; menu.style.position = 'absolute'; menu.style.right = '12px'; menu.style.top = '120px'; menu.style.background = 'linear-gradient(180deg,#111,#000)'; menu.style.border = '1px solid #222'; menu.style.borderRadius = '6px'; menu.style.padding = '6px'; menu.style.zIndex = 30000; menu.style.boxShadow = '0 8px 40px rgba(0,0,0,0.6)'; menu.style.color = '#fff'; menu.style.minWidth = '140px';
    ...
    function refreshLocalizedText(){ try{ // update hud (buttons, labels)
        ...
        try{ const titleEl = document.getElementById('tn-title'); if (titleEl) titleEl.innerHTML = `<div class="tn-logo">${t('tank_nutz')}</div>`; }catch(_){ }
  ...
  // Update weapon phase and cooldown elements in controls panel
  try{
    const phaseDiv = document.getElementById('tn-phase-div');
    if (phaseDiv) {
      phaseDiv.innerHTML = `<div class="tn-panel tn-phase" style="margin-top:4px"><div class="tn-shoot-dots">${dots}</div><div style="font-size:12px;color:#ccc;margin-top:4px">${phaseLabel}</div></div>`;
    }
    const cooldownDiv = document.getElementById('tn-cooldown-div');
    if (cooldownDiv) {
      const pieces = [];
      if (shieldInfo) pieces.push(shieldInfo);
      if (envCooldownInfo) pieces.push(envCooldownInfo);
      cooldownDiv.innerHTML = pieces.length ? pieces.join('') : `<div class="tn-panel tn-cooldown-placeholder" style="margin-top:4px;padding:6px 10px;font-size:12px;color:#d7cdbf">Cooldowns: None</div>`;
    }
    const powerupDiv = document.getElementById('tn-powerup-div');
    if (powerupDiv) {
      powerupDiv.innerHTML = pu || '';
    }
    const waveDiv = document.getElementById('tn-wave-div');
    if (waveDiv) {
      waveDiv.innerHTML = `<div class="tn-panel" style="margin-top:4px;font-size:12px;color:#ffd700">${waveInfo}</div>`;
    }
    const scoreDiv = document.getElementById('tn-score-div');
    if (scoreDiv) {
      scoreDiv.innerHTML = `<div class="tn-panel" style="margin-top:4px"><div class="tn-score">${t('score')}: ${score}</div></div>`;
    }
  }catch(_){ }
}
...
function showControlsModal(){
  ...
  inner.appendChild(row(keyboardSvg, t('spacebar_hold') || 'Spacebar (hold)', t('pause_vertical_scrolling')));
...
const dx = mouseWorldX - heli.x, dy = mouseWorldY - heli.y; heli.turretAngle = Math.atan2(dy, dx);
  ...
  if (e.harmless){ enemies.splice(ei,1); recordDeath(e.kind || '<harmless>', e); try{ recordKill(e.kind || (typeof e.spriteIdx === 'number' ? ('animal-' + e.spriteIdx) : '<harmless>'), e); }catch(_){ } score += 1; updateHud(); callSpawnGibs(e.x, e.y, '#ff8b6b', 8, 'enemy-hit-harmless'); try{ enemyDropChance(e,e.x,e.y); }catch(_){ } }
        ...
        e.hp = (e.hp||1) - 1;
  if (e.hp <= 0){ try{ recordDeath(e.kind || (typeof e.spriteIdx === 'number' ? ('animal-' + e.spriteIdx) : '<killed>'), e); }catch(_){ } try{ recordKill(e.kind || (typeof e.spriteIdx === 'number' ? ('animal-' + e.spriteIdx) : '<killed>'), e); }catch(_){ } enemies.splice(ei,1); score += 1; updateHud(); callSpawnGibs(e.x, e.y, '#ff8b6b', 12, 'enemy-killed-by-bullet'); try{ enemyDropChance(e,e.x,e.y); }catch(_){ } }
  } else {
  ...
  // update pet tanks
  if (petTanks.length) updatePetTanks(dt);
  ...
  for (let pi = powerups.length-1; pi >= 0; pi--){ const p = powerups[pi]; const vx = (selectedVehicle === 'heli') ? heli.x : tank.x; const vy = (selectedVehicle === 'heli') ? heli.y : tank.y; const dy = (selectedVehicle === 'heli') ? (vy - p.y) : wrapDeltaY(vy, p.y); const dx = vx - p.x; const pickR = (p.type === 'heal' || p.type === 'journal') ? 28 : 20; if (Math.hypot(dx, dy) < pickR){ // picked up
      if (p.type === 'double'){
        ...
        try{ updateHud(); }catch(_){ }
        // Reset environmental powerup cooldown when any powerup is collected
        ...
        if (nowt > shieldCooldownUntil){
          ...
          // Reset environmental powerup cooldown even when shield is on cooldown
          envPowerupCooldown = 0;
        }
        powerups.splice(pi,1);
      }
...
// legacy `grassCanvas` drawing removed when foliage was extracted to `js/foliage.js`.
  ...
  // Draw pet tanks
  try{ drawPetTanks(ctx, worldToScreen, camera); }catch(_){ }
  ...
  // draw critters and animals
  try{ drawCritters(ctx, camera, now); }catch(_){ }
  try{ drawAnimals(ctx, camera, now); }catch(_){ }
