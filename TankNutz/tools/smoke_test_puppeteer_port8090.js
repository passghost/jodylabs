...
(async ()=>{
  ...
  // create pure plain-object snapshots inside the page so Puppeteer gets serializable data
  try{
    await page.evaluate(()=>{
      try{
        if (Array.isArray(window.critters)) window.__critters_serializable = window.critters.slice(0,6).map(c=>({ x: c.x, y: c.y, kind: c.kind }));
        if (Array.isArray(window.animals)) window.__animals_serializable = window.animals.slice(0,6).map(a=>({ x: a.x, y: a.y, kind: a.kind }));
      }catch(_){ }
    });
  }catch(_){ }
  // read one-shot diag object exposed by critters module (if present)
  try{
    const diag = await page.evaluate(()=>{ try{ return window.__critters_diag || null; }catch(_){ return null; } });
    console.log('[DIAG]', JSON.stringify(diag));
  }catch(_){ }
  try{
    const diag2 = await page.evaluate(()=>{ try{ return window.__critters_diag_spawn || null; }catch(_){ return null; } });
    console.log('[DIAG_SPAWN]', JSON.stringify(diag2));
  }catch(_){ }
  try{
    const spawnSnap = await page.evaluate(()=>{ try{ return window.__critters_spawn_snapshot || null; }catch(_){ return null; } });
    console.log('[SPAWN_SNAPSHOT]', JSON.stringify(spawnSnap));
  }catch(_){ }
  try{
    const aspawn = await page.evaluate(()=>{ try{ return window.__animals_spawn_snapshot || null; }catch(_){ return null; } });
    console.log('[ANIMALS_SPAWN_SNAPSHOT]', JSON.stringify(aspawn));
  }catch(_){ }
  try{
    // Build a compact summary using data exposed by the page and module internals when available
  ...
  const serA = (window.__animals_serializable || null);
        // fallback to reading first few module samples if present (not guaranteed on all builds)
        ...
        // Also try to read visible arrays from module scope (may be undefined)
        try{ if (window.critters && Array.isArray(window.critters)) raw.critters = raw.critters.concat(window.critters.slice(0,6).map(c=>({x:c.x,y:c.y,kind:c.kind}))).slice(0,6); }catch(_){ }
        try{ if (window.animals && Array.isArray(window.animals)) raw.animals = raw.animals.concat(window.animals.slice(0,6).map(a=>({x:a.x,y:a.y,kind:a.kind}))).slice(0,6); }catch(_){ }
  ...
  // Capture the canvas backing as a PNG data URL and write it to disk for visual inspection
  try{
    ...
    if (dataUrl){
      ...
      const outPath = path.join(ROOT, 'tmp_canvas.png');
      fs.writeFileSync(outPath, Buffer.from(base64, 'base64'));
      console.log('[CANVAS_SAVED]', outPath);
    } else {
      console.log('[CANVAS_SAVED]', 'no-canvas');
    }
